...
public extension MoyaProvider {
    /// json
    func requestJson(_ token: Target, isCache: Bool = false) -> RxSwift.Observable<Any> {
    ...
    /// string
    func requestString(_ token: Target, isCache: Bool = false) -> RxSwift.Observable<String> {
        return sb_request(token, isCache: isCache)
            .filterSuccessfulStatusCodes()
            .mapString()
    }
